...
#
datosDepartamentos = pd.read_excel('DatosMaestrosDepartamento.xlsx')
Departamento = st.sidebar.selectbox('Elegir departamento', options = datosDepartamentos)
...
if(st.sidebar.button("Cargar")):
    ...
    datosExcel = pd.read_excel('OrigenDatosPoblacion.xlsx')
    DatosFiltrados = datosExcel.loc[datosExcel['Departamento'] == Departamento]
    datosAgrupados = DatosFiltrados.groupby(['Año','Departamento'], as_index = False)['Total'].sum()
    datos = pd.DataFrame(datosAgrupados)
    st.write(datos)

    # Al graficar los datos se observa una tendencia lineal
    st.set_option('deprecation.showPyplotGlobalUse', False)
    datos.plot.scatter(x='Año', y='Total')
    plt.xlabel('Año')
    plt.ylabel('Total')
    plt.show()
